...
namespace HandtrackLibraryUtilities
{
    ...
    public class HandsController
    {
        ...
        /// </summary>
        public HandsController(bool detectLeftHand, bool detectRightHand)
        {
            CoordinateMapper = KinectSensor.GetDefault().CoordinateMapper;
            DetectLeftHand = detectLeftHand;
            DetectRightHand = detectRightHand;
        }
        ...
        public unsafe void Update(ushort* data, Body body)
        {
            ...
            float handLeftZ = jointHandLeft.Position.Z;
            ...
            float handRightZ = jointHandRight.Position.Z;
            ...
            if (searchForLeftHand || searchForRightHand)
            {
                ...
                if (searchForLeftHand)
                {
                    handLeft = GetHand(body.TrackingId, "left", contourLeft, angleLeft, wristLeftX, wristLeftY, handLeftX, handLeftY, handLeftZ);
                }

                if (searchForRightHand)
                {
                    handRight = GetHand(body.TrackingId, "right", contourRight, angleRight, wristRightX, wristRightY, handRightX, handRightY, handRightZ);
                    //Console.WriteLine("Fingers " + handRight.Fingers.Count);
                }
            }
        ...
        private double Distance(float xa, float ya, float xb, float yb)
        {
        ...
        private Hand GetHand(ulong trackingID, string side, List<DepthPointEx> contour, double angle, float wristX, float wristY, float handX, float handY, float handZ)
        {
            ...
            float distance = 30 / Convert.ToSingle(Math.Pow(handZ, 1.7F));
            distance = distance > 30 ? 30 : distance;
            float offsetx = -3;
            float offsety = 0;
            IList<DepthPointEx> range = new List<DepthPointEx>();

            if (side.Equals("right"))
            {
                fingers = filtered.Where(p => p.Y < wristY && Distance(p.X, p.Y, handX - offsetx, handY + offsety) > distance && !(p.X > handX - distance && p.Y > handY)).Take(5).ToList();

                range.Add(new DepthPointEx { X = handX - distance, Y = handY, Z = handZ }); //extra point left top

                range.Add(new DepthPointEx { X = handX - offsetx, Y = handY - distance + offsety, Z = handZ });
                range.Add(new DepthPointEx { X = handX - offsetx + (float)Math.Cos(45) * distance, Y = handY - (float)Math.Sin(45) * distance + offsety, Z = handZ });
                range.Add(new DepthPointEx { X = handX - offsetx + (float)Math.Cos(135) * distance, Y = handY - (float)Math.Sin(135) * distance + offsety, Z = handZ });

                range.Add(new DepthPointEx { X = handX + distance - offsetx, Y = handY + offsety, Z = handZ });
                range.Add(new DepthPointEx { X = handX - distance - offsetx, Y = handY + offsety, Z = handZ });
            }
            else
            {
                fingers = filtered.Where(p => p.Y < wristY && Distance(p.X, p.Y, handX + offsetx, handY + offsety) > distance && !(p.X < handX + distance && p.Y > handY)).Take(5).ToList();
                range.Add(new DepthPointEx { X = handX + distance, Y = handY, Z = handZ }); //extra point right top
            ...
            {
                return new Hand(trackingID, contour, fingers, CoordinateMapper, new DepthPointEx { X = handX - offsetx, Y = handY + offsety, Z = handZ }, range);
            }
