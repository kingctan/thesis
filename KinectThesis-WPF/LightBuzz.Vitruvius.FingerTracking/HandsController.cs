    ...
    public class HandsController
    {
        ...
        public unsafe void Update(ushort* data, Body body)
        {
            ...
            if (searchForLeftHand || searchForRightHand)
            {
                ...
                if (searchForLeftHand)
                {
                    handLeft = GetHand(body.TrackingId, body.HandLeftState, contourLeft, angleLeft, wristLeftX, wristLeftY, handLeftX, handLeftY);
                }

                if (searchForRightHand)
                {
                    handRight = GetHand(body.TrackingId, body.HandRightState, contourRight, angleRight, wristRightX, wristRightY, handRightX, handRightY);
                }
            }
        ...
        private Hand GetHand(ulong trackingID, HandState state, List<DepthPointEx> contour, double angle, float wristX, float wristY, float handX, float handY)
        {
            ...
            IList<DepthPointEx> fingersNew = new List<DepthPointEx>();
            fingers = filtered.Where(p => p.Y < wristY && Distance(p.X, p.Y, handX, handY) > 30).Take(5).ToList();
